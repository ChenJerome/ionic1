...
<div class="modal-header">
	...
	<h4 class="modal-title">查看${userInfo.userName}所属角色</h4>
</div>
<div class="main-box-body clearfix" style="overflow: visible;">
	<input type="hidden" id="userCode" value="${userInfo.userCode}">
	<table class="table table-striped table-hover text-center">
		...
		<tbody id="privilegeTbd">
			<c:choose>
				<c:when
					test="${roleMemberPage.recordList != null && roleMemberPage.recordList.size() > 0}">
					<form id="submitForm"
						action="${pageContext.request.contextPath}/authMenuAction/delete"
						method="post">
						<c:forEach var="roleRecord" items="${roleMemberPage.recordList}"
							varStatus="status">
							<tr id="listDataTr_${roleRecord.code}">
								<td><input type="checkbox" value="${roleRecord.code}"></td>
								<td>${roleRecord.roleName}</td>
								<td class="text-center">
									<a href="#" type="button" onclick="deleteRoleData(${roleRecord.code})" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></a>
								</td>
							</tr>
						</c:forEach>
					</form>
...
function addRoleMember(){
	...
	selectRoleId.push($("#userCode").val());
	...
	if( selectRoleId.length == 1 ){
	 ...
	 $.ajax({
		url : "${pageContext.request.contextPath}/AuthUserInfoAction/addRoleMember",
		type : 'POST',
		dataType:"json",
		...
		success : function(data) {
			if(data.length>0){
				...
				for (var i = 0; i < data.length; i++) {
					...
					var trObj = $("<tr id='listDataTr_"+privilegeObj.code+"'><td><input type='checkbox' value='"+privilegeObj.code+"'></td><<td>"+privilegeObj.roleName+"</td><td class='text-center'><a href='#' type='button' onclick='deleteRoleData("+privilegeObj.code+")' class='btn btn-danger btn-xs'><i class='fa fa-trash-o'></i></a></td></tr>");
					talObj.append(trObj);
				}
			}
		}
