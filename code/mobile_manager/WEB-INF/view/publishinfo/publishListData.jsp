...
<header class="main-box-header clearfix">
	<div class="row m-b-sm m-t-sm">
		<div class="col-xs-2">
			 <a data-target="#commonWin" class="btn btn-primary btn-sm" type="button" data-toggle="modal" href="${pageContext.request.contextPath}/pubilshInfoAction/add"><i class="fa fa-plus-circle"></i>&nbsp;新增版本</a>
		</div>
	...
	<table class="table table-striped table-hover text-center">
		<thead>
			<tr>
				...
				<th class="text-center">是否最新版本</th>
				<th class="text-center">新建日期</th>
					...
					<form id="submitForm" action="${pageContext.request.contextPath}/userAction/delete" method="post">
						<c:forEach var="record" items="${page.recordList}" varStatus="status">
		               		<tr>
								...
								<td>${record.editionCode}</td>
								<td>${record.changeLog}</td>
								<td><code:getDisText category="YESORNO" codeValue="${record.isLatest}"></code:getDisText></td>
								<td>${record.createTime}</td>
								<td>${record.updateTime}</td>
								<td class="text-center">
									<a type="button" href="${pageContext.request.contextPath}/pubilshInfoAction/downloadPic/${record.code}" class="btn btn-primary btn-xs"><i class="fa fa-barcode"></i></a>	
									<a href="${pageContext.request.contextPath}/pubilshInfoAction/modfiy/${record.code}" data-target="#commonWin" title="编辑" data-toggle="modal" class="btn btn-primary btn-xs"><i class="fa fa-pencil"></i></a>
