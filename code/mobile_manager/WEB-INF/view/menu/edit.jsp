...
<form action="${pageContext.request.contextPath}/menuAction/add" id="addForm" role="form" class="form-horizontal" method="post">
		...
		<div class="row" style="max-height: 600px; overflow: scroll-y;">
			<div class="col-xs-12">
				...
				<div class="form-group">
					...
					<div class="col-xs-3">
						<select class="form-control" name="parent.menuId" id="parent">
							<option value="0" <c:if test="${0 == record.parent.menuId}">selected</c:if>>无</option>
							<c:forEach var="menu" items="${menuList}">
								<option value="${menu.menuId}" <c:if test="${menu.menuId == record.parent.menuId}">selected</c:if>>${menu.menuText}</option>
							</c:forEach>
						</select>
					</div>
					<label class="col-xs-2 control-label"><label class="text-danger" for="menuType">*</label> 菜单类型： </label>
					<div class="col-xs-3">
						<code:select category="menuType" selectClass="form-control" name="menuType" id="menuType" selectValue="${record.menuType}"/>
					</div>
				</div>
				<div class="form-group ">
					<div class="col-xs-1 formCtrl"></div>
					...
					<div class="col-xs-3">
						<input name="menuText" id="menuText" value="${record.menuText}" placeholder="请输入菜单名称" type="text" class="form-control " />
					</div>
				...
				<div class="form-group">
					...
					<div class="col-md-3">
						<input name="odBy" id="odBy" value="${record.odBy}" placeholder="请输入菜单排序" type="text" class="form-control " />
					</div>
				...
				<div class="form-group">
					<div class="col-xs-1 formCtrl"></div>
					<label class="col-xs-2 control-label">菜单主体：</label>
					<div class="col-xs-8">
						<input name="menuBody" id="menuBody" value="${record.menuBody}" placeholder="请输入菜单主体" type="text" class="form-control " />
					</div>
				</div>
